...
> 本文档面向 UI 设计师、前端开发工程师、测试工程师，描述机器翻译页面的产品概述、用户场景、字段约束、交互逻辑、状态流转、异常处理与性能需求。所有内容基于原型页面真实可验证的元素生成。
...
|-------------|------|------|---------------|
| 品牌标识 | 图标 + 文字 | — | 渐变图标 + 「AI Hub」字样，常驻显示 |
...
| 状态指示器 | 状态标签（带圆点） | — | 反映翻译状态（详见第五章）；圆点颜色随状态变化：就绪/完成为绿色，翻译中为黄色，拦截/空内容为红色 |
...
| 操作项目 | 显示/触发条件 | 点击行为 | 校验规则 | 成功反馈 | 失败反馈 | 状态流转 |
|---------|-------------|---------|---------|---------|---------|---------|
| 主题切换 | 始终可点击 | 深色 ⇄ 浅色主题切换，图标随之变化 | 无 | 无 | 无 | 主题状态切换 |
| 界面语言切换 | 始终可点击 | 中文 ⇄ 英文切换，所有界面文案同步更新 | 无 | 无 | 无 | 界面语言切换 |

### 4.2 语言与领域配置操作

| 操作项目 | 显示/触发条件 | 点击行为 | 校验规则 | 成功反馈 | 失败反馈 | 状态流转 |
|---------|-------------|---------|---------|---------|---------|---------|
| 源语言选择 | 点击源语言选择器展开菜单 | 选中某项后更新源语言，关闭菜单 | 无 | 无 | 无 | 源语言变更 |
| 目标语言选择 | 点击目标语言选择器展开菜单 | 选中某项后更新目标语言，译文标题同步更新 | 无 | 无 | 无 | 目标语言变更 |
| 语言互换 | 点击语言互换按钮 | 交换源语言与目标语言，保留原文内容；源为自动检测时取中文作为新目标 | 无 | 无 | 无 | 源/目标语言互换 |
| 专业领域选择 | 点击专业领域选择器展开菜单 | 更新领域；若已有译文则按新领域重新翻译 | 无 | 无 | 无 | 领域变更，可能触发重新翻译 |

### 4.3 翻译操作

| 操作项目 | 显示/触发条件 | 点击行为 | 校验规则 | 成功反馈 | 失败反馈 | 状态流转 |
|---------|-------------|---------|---------|---------|---------|---------|
| 立即翻译 | 始终显示；翻译执行期间禁用 | 校验输入 → 前置风控检测 → 执行翻译 → 展示结果 | 原文为空→拦截；超 2000 字→拦截；敏感词命中→拦截 | 译文展示在结果区 | 见异常处理 | 已就绪→翻译中→已完成/已拦截/请输入内容 |
| 清空 | 始终可点击 | 清空原文、字数归零、清空译文、隐藏错误提示、聚焦输入框 | 无 | 无 | 无 | 回到已就绪 |

### 4.4 译文结果操作

| 操作项目 | 显示/触发条件 | 点击行为 | 校验规则 | 成功反馈 | 失败反馈 | 状态流转 |
|---------|-------------|---------|---------|---------|---------|---------|
| 朗读译文 | 结果区有内容时有效 | 调用语音合成朗读译文；朗读中再次点击停止 | 无结果时不响应 | 无 | 浏览器不支持时提示 | 朗读中状态开关 |
| 复制译文 | 结果区有内容时有效 | 过滤脱敏符号后写入剪贴板；剪贴板 API 失败时降级兼容方式复制 | 无结果时不响应 | 「复制成功」 | 「复制失败」 | 无 |

### 4.5 敏感词管理操作（弹窗内）

| 操作项目 | 显示/触发条件 | 点击行为 | 校验规则 | 成功反馈 | 失败反馈 | 状态流转 |
|---------|-------------|---------|---------|---------|---------|---------|
| 打开弹窗 | 点击敏感词管理按钮 | 显示弹窗，渲染敏感词列表，聚焦新增输入框 | 无 | 无 | 无 | 弹窗显示 |
| 搜索敏感词 | 在搜索框输入文本 | 实时过滤列表，仅显示匹配项；显示总数不变 | 无 | 无 | 无 | 列表过滤 |
| 新增敏感词 | 在新增输入框输入并回车 | 将敏感词加入列表，清空输入框 | 不能为空；不能与已有重复 | 「已添加」 | 「请输入敏感词」/「该敏感词已存在」 | 列表项增加 |
| 编辑敏感词 | 点击列表项编辑按钮 | 该行变为可编辑输入框，聚焦并全选当前内容 | 无 | 无 | 无 | 该行进入编辑态 |
| 保存编辑 | 编辑态下回车或点击保存按钮 | 校验后更新该敏感词 | 不能为空；不能与已有重复 | 「已更新」 | 「敏感词不能为空」/「该敏感词已存在」 | 该行退出编辑态，内容更新 |
| 取消编辑 | 编辑态下点击取消按钮或按 Esc | 退出编辑态，恢复原值 | 无 | 无 | 无 | 该行退出编辑态 |
| 删除敏感词 | 点击列表项删除按钮 | 从列表移除该敏感词 | 无 | 「已删除」 | 无 | 列表项减少 |
| 保存并关闭 | 点击保存按钮 | 保存所有变更，关闭弹窗，更新风控列表与底部信息条；若已有译文则重新翻译 | 无 | 无 | 无 | 弹窗关闭，风控列表更新 |
| 取消并关闭 | 点击取消按钮或关闭按钮 | 关闭弹窗，取消所有进行中的行内编辑 | 无 | 无 | 无 | 弹窗关闭 |
