...
|---------|-------------|---------|---------|---------|---------|---------|
| 源语言选择 | 选择器展开后选中某项 | 更新源语言，关闭菜单 | 无 | 无 | 无 | 源语言变更 |
| 目标语言选择 | 选择器展开后选中某项 | 更新目标语言，译文标题同步更新 | 无 | 无 | 无 | 目标语言变更 |
| 语言互换 | 互换按钮被点击 | 交换源语言与目标语言，保留原文内容；源为自动检测时取中文作为新目标 | 无 | 无 | 无 | 源/目标语言互换 |
| 专业领域选择 | 选择器展开后选中某项 | 更新领域；若已有译文则按新领域重新翻译 | 无 | 无 | 无 | 领域变更，可能触发重新翻译 |
...
| 操作项目 | 显示/触发条件 | 点击行为 | 校验规则 | 成功反馈 | 失败反馈 | 状态流转 |
|---------|-------------|---------|---------|---------|---------|---------|
| 打开弹窗 | 敏感词管理按钮被点击 | 渲染敏感词列表，聚焦新增输入框 | 无 | 无 | 无 | 弹窗显示 |
| 搜索敏感词 | 搜索框输入文本 | 实时过滤列表，仅显示匹配项；显示总数不变 | 无 | 无 | 无 | 列表过滤 |
| 新增敏感词 | 新增输入框输入并回车 | 将敏感词加入列表，清空输入框 | 不能为空；不能与已有重复 | 「已添加」 | 「请输入敏感词」/「该敏感词已存在」 | 列表项增加 |
| 编辑敏感词 | 列表项编辑按钮被点击 | 该行变为可编辑输入框，聚焦并全选当前内容 | 无 | 无 | 无 | 该行进入编辑态 |
| 保存编辑 | 编辑态下回车或点击保存按钮 | 校验后更新该敏感词 | 不能为空；不能与已有重复 | 「已更新」 | 「敏感词不能为空」/「该敏感词已存在」 | 该行退出编辑态，内容更新 |
| 取消编辑 | 编辑态下点击取消按钮或按 Esc | 退出编辑态，恢复原值 | 无 | 无 | 无 | 该行退出编辑态 |
| 删除敏感词 | 列表项删除按钮被点击 | 从列表移除该敏感词 | 无 | 「已删除」 | 无 | 列表项减少 |
| 保存并关闭 | 保存按钮被点击 | 保存所有变更，关闭弹窗，更新风控列表与底部信息条；若已有译文则重新翻译 | 无 | 无 | 无 | 弹窗关闭，风控列表更新 |
| 取消并关闭 | 取消按钮或关闭按钮被点击 | 关闭弹窗，取消所有进行中的行内编辑 | 无 | 无 | 无 | 弹窗关闭 |
